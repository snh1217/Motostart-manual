  ...
  return (
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex flex-wrap items-center gap-2">
        <h2 className="text-base font-semibold">엑셀/CSV 업로드</h2>
        <input
          type="password"
          value={adminToken}
      ...
      <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
        <p>지원 컬럼 예시: model, name(part), system, summary, tags, photo_url, photo_label,</p>
        <p>step_order, step_title, step_desc, step_tools, step_torque, step_note</p>
        <p>같은 부품 이름/모델을 가진 행을 묶어 사진과 단계를 병합합니다.</p>
      </div>

      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <h3 className="text-sm font-semibold">사진 업로드 (Supabase Storage)</h3>
        <form onSubmit={uploadPhoto} className="mt-2 space-y-2">
