...
export async function POST(request: Request) {
  ...
  const partId = payload?.partId?.trim() || "part";
  ...
  const objectPath = `parts/${safeSegment(model)}/${safeSegment(partId)}/${Date.now()}-${safeName}`;
  ...
  const { data: publicUrl } = supabaseAdmin.storage.from("parts").getPublicUrl(objectPath);
